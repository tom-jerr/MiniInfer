...
def scaled_dot_product_attention_simple(
  ...
  """
  q_k = torch.matmul(query, key.transpose(-2, -1))  # (batch_size, num_heads, seq_len_q, seq_len_k)
  scores = q_k / (
...
class SimpleMultiHeadAttention(nn.Module):
  ...
  def forward(
    ...
    projection_k = linear(key, self.wk).reshape(N, L, self.num_heads, self.head_dim).transpose(1, 2)
    projection_v = (
...
def scaled_dot_product_attention_grouped(
  ...
  key_f = key.float()
  value_f = value.float()
  q_k = torch.matmul(query_f, key_f.transpose(-2, -1))  # [B, H_kv, n_repeats, S_q, S_k]
