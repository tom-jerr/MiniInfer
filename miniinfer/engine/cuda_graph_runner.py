...
class CudaGraphRunner:
  ...
  def _create_static_buffers(self, batch_size: int) -> CapturedGraph:
    ...
    return CapturedGraph(
      ...
      output_logits=torch.zeros(batch_size, self.vocab_size, dtype=self.dtype, device=device),
  ...
  def warmup(self) -> None:
    ...
    """
    logger.info(f"Starting CUDA graph warmup for batch sizes: {self.capture_batch_sizes}")
  ...
  def replay(self, forward_batch: "ForwardBatch") -> "BaseModelOutput":
    ...
    if actual_bs < padded_bs:
      ...
      captured.req_pool_indices[actual_bs:padded_bs] = captured.req_pool_indices[0]
