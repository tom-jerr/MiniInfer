...
def apply_rotary_emb(
  ...
  if offset is None:
    ...
    # just one slice
    assert offset.stop - offset.start == seq_len, "Offset slice length must match sequence length"
...
class NaiveAttnBackend:
  ...
  def forward(
    ...
    **kwargs,
    ...
    if batch_size == 1:
      seq_len = seq_lens[0].item() if isinstance(seq_lens, torch.Tensor) else seq_lens[0]
...
def create_simple_forward_batch(
  ...
  batch = ForwardBatch(
    ...
    out_cache_loc=torch.zeros(batch_size * seq_len, dtype=torch.long, device=device),
    seq_lens=seq_lens,
