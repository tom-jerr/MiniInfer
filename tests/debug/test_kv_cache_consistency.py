...
def _should_apply_chat_template(tokenizer, model_name: str, use_chat_template: str) -> bool:
...
def _compare_step(
  ...
  logits_close = torch.allclose(logits_a_f, logits_b_f, rtol=logit_rtol, atol=logit_atol)
  ...
  hidden_close = torch.allclose(hidden_a_f, hidden_b_f, rtol=hidden_rtol, atol=hidden_atol)
  cos_sim = torch.nn.functional.cosine_similarity(hidden_a_f, hidden_b_f, dim=0).item()
  ...
  if not top1_match:
    print(f"  top1 A={_fmt_token(tokenizer, top1_a)} " f"B={_fmt_token(tokenizer, top1_b)}")
...
def _compare_against_hf(
  ...
  logits_close = torch.allclose(logits_ref_f, logits_hf_f, rtol=logit_rtol, atol=logit_atol)
  ...
  hidden_close = torch.allclose(hidden_ref_f, hidden_hf_f, rtol=hidden_rtol, atol=hidden_atol)
  cos_sim = torch.nn.functional.cosine_similarity(hidden_ref_f, hidden_hf_f, dim=0).item()
  ...
  if not top1_match:
    print(
      f"    top1 ours={_fmt_token(tokenizer, top1_ref)} " f"hf={_fmt_token(tokenizer, top1_hf)}"
    )
...
def _compare_hidden_layers(
  ...
  for idx, (h_ours, h_hf) in enumerate(zip(ours_aligned, hf_aligned)):
    ...
    max_abs = torch.max(diff).item()
    mean_abs = torch.mean(diff).item()
    close = torch.allclose(h_ours_last.float(), h_hf_last.float(), rtol=rtol, atol=atol)
    all_ok = all_ok and close
    print(f"    layer[{idx:02d}] close={close} max_abs={max_abs:.4e} " f"mean_abs={mean_abs:.4e}")
  return all_ok
...
def _debug_compare_kv_last_token(
  ...
  token_a = int(batch_a.input_ids[0].item()) if batch_a.input_ids is not None else None
  token_b = int(batch_b.input_ids[-1].item()) if batch_b.input_ids is not None else None
  print(
  ...
  for layer_id in range(num_layers):
    ...
    if not (k_close and v_close):
      ...
      if spy_a is not None and spy_b is not None and layer_id in spy_a and layer_id in spy_b:
        ...
        k_in_close = torch.allclose(k_a_in.float(), k_b_in.float(), rtol=rtol, atol=atol)
        v_in_close = torch.allclose(v_a_in.float(), v_b_in.float(), rtol=rtol, atol=atol)
        print(f"[KV][in] layer[{layer_id:02d}] k_in_close={k_in_close} v_in_close={v_in_close}")
        # Compare what we tried to write vs what ended up in storage.
        k_a_store_close = torch.allclose(k_a_in.float(), k_a_t.float(), rtol=rtol, atol=atol)
        v_a_store_close = torch.allclose(v_a_in.float(), v_a_t.float(), rtol=rtol, atol=atol)
        k_b_store_close = torch.allclose(k_b_in.float(), k_b_t.float(), rtol=rtol, atol=atol)
        v_b_store_close = torch.allclose(v_b_in.float(), v_b_t.float(), rtol=rtol, atol=atol)
        print(
      ...
      if layer_id == 0 and model is not None and token_a is not None:
        try:
          ...
          q_ref, k_ref, v_ref = qkv.split([attn0.q_size, attn0.kv_size, attn0.kv_size], dim=-1)
          q_ref, k_ref = attn0.rope(pos, q_ref, k_ref)
          k_ref = k_ref.view(-1, attn0.num_kv_heads, attn0.head_dim)[0].float()
          v_ref = v_ref.view(-1, attn0.num_kv_heads, attn0.head_dim)[0].float()
  ...
  for i, req_idx in enumerate(req_indices):
    ...
    for j in range(1, len(kv_locs)):
      ...
      if cur_page == prev_page:
        if cur != prev + 1:
          print(f"[map][{stage}] req_idx={req_idx} break at pos {j}: " f"{prev}->{cur} (same page)")
          issues += 1
          break
      elif cur_page == prev_page + 1:
        if (prev % page_size) != (page_size - 1) or (cur % page_size) != 0:
          print(
            f"[map][{stage}] req_idx={req_idx} break at pos {j}: " f"{prev}->{cur} (page boundary)"
          )
          issues += 1
          break
      else:
    ...
    if stage == "decode" and batch.out_cache_loc is not None:
      ...
      if kv_locs and kv_locs[-1] != expected:
        print(f"[map][decode] req_idx={req_idx} last_loc mismatch: " f"{kv_locs[-1]} != {expected}")
        issues += 1
    if stage == "extend" and extend_last_locs is not None:
      ...
      if expected is not None and kv_locs and kv_locs[-1] != expected:
        print(f"[map][extend] req_idx={req_idx} last_loc mismatch: " f"{kv_locs[-1]} != {expected}")
        issues += 1
...
def main():
  ...
  parser.add_argument("--show-topk", action="store_true", help="Print top-k tokens each step")
  ...
  if debug_mapping:
    _debug_check_req_to_token_pool(kv_mgr_a, batch_a, page_size=page_size, stage="extend")
  out_prefill = _forward_with_backend(model_runner.model, attn_backend_a, forward_batch_a)
  ...
  for step_idx in range(1, args.steps + 1):
    ...
    forward_batch_a.debug_use_past_len_cache = debug_decode_past_len
    if debug_mapping:
      _debug_check_req_to_token_pool(kv_mgr_a, batch_a, page_size=page_size, stage="decode")
    if args.debug_compare_kv:
      act_spy["tag"] = "A"
    ...
    if debug_mapping:
      _debug_check_req_to_token_pool(kv_mgr_b, batch_b, page_size=page_size, stage="extend")
    if args.debug_compare_kv:
      act_spy["tag"] = "B"
    ...
    if args.compare_hf or args.compare_hf_layers:
      ...
      if args.compare_hf_layers:
        ours_layers = _collect_hidden_layers(out_b.hidden_states, out_b.last_hidden_state)
    ...
    if debug_compare_ab_layers and out_a.hidden_states is not None:
      out_a_layers = _collect_hidden_layers(out_a.hidden_states, out_a.last_hidden_state)
      out_b_layers = _collect_hidden_layers(out_b.hidden_states, out_b.last_hidden_state)
