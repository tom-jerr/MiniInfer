...
def test_rmsnorm(model_name: str, device, layer_idx=0):
  ...
  # HF RMSNorm
  hf_norm = HFQwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps).to(device, torch.float16)
...
def test_mlp(model_name: str, device, layer_idx=0):
  ...
  hf_mlp.down_proj.weight.data = state_dict[f"{prefix}.down_proj.weight"].to(device, torch.float16)
...
def test_attention(model_name: str, device, layer_idx=0):
  ...
  hf_attn.q_proj.weight.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.q_proj.weight"].to(
    device, torch.float16
  )
  hf_attn.q_proj.bias.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.q_proj.bias"].to(
    device, torch.float16
  )
  hf_attn.k_proj.weight.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.k_proj.weight"].to(
    device, torch.float16
  )
  hf_attn.k_proj.bias.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.k_proj.bias"].to(
    device, torch.float16
  )
  hf_attn.v_proj.weight.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.v_proj.weight"].to(
    device, torch.float16
  )
  hf_attn.v_proj.bias.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.v_proj.bias"].to(
    device, torch.float16
  )
  hf_attn.o_proj.weight.data = raw_state_dict[f"{p}layers.{layer_idx}.self_attn.o_proj.weight"].to(
    device, torch.float16
  )
  ...
  # Test data
  batch_size, seq_len = 1, 10
  x = torch.randn(batch_size, seq_len, config.hidden_size, dtype=torch.float16, device=device)
  positions = torch.arange(seq_len, device=device)
  ...
  try:
    assert_allclose(my_output, hf_output, precision=torch.float16, rtol=2e-1, atol=2e-1)
...
def test_full_model(model_name: str, device):
  ...
  with torch.no_grad():
    ...
    my_output = my_model(input_ids, positions=positions, forward_batch=forward_batch).logits
    my_output = my_output.reshape(batch_size, seq_len, -1)
  ...
  try:
    assert_allclose(my_output, hf_output, precision=torch.float16, rtol=1e-1, atol=1e-1)
    print("✓ 完整模型精度测试通过")
    return True
  except Exception as e:
