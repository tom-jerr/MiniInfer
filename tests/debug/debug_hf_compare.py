...
def compare_weights(model_path: str, device: str = "cuda"):
    ...
    print(
        f"  QKV weight max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    print(
        f"  HF QKV bias shape: {hf_qkv_bias.shape}, MI QKV bias shape: {mi_qkv_bias.shape}"
    )
    diff = (hf_qkv_bias - mi_qkv_bias).abs()
    print(
        f"  QKV bias max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    diff = (hf_o - mi_o).abs()
    print(
        f"  O_proj weight max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    print(
        f"  Gate+Up weight max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    print(
        f"  Down weight max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    print(
        f"  Input LN max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    print(
        f"  Post-attn LN max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
    ...
    print(
        f"  Final norm max diff: {diff.max().item():.6e}, mean diff: {diff.mean().item():.6e}"
    )
...
def compare_forward(hf_model, mi_model, input_ids, device: str = "cuda"):
    ...
    print(
        f"  Q max diff: {diff_q.max().item():.6e}, mean diff: {diff_q.mean().item():.6e}"
    )
    print(
        f"  K max diff: {diff_k.max().item():.6e}, mean diff: {diff_k.mean().item():.6e}"
    )
    print(
        f"  V max diff: {diff_v.max().item():.6e}, mean diff: {diff_v.mean().item():.6e}"
    )
    ...
    # MiniInfer RoPE
    mi_q_reshaped = mi_q.view(
        seq_len, mi_layer0.self_attn.num_heads, mi_layer0.self_attn.head_dim
    )
    mi_k_reshaped = mi_k.view(
        seq_len, mi_layer0.self_attn.num_kv_heads, mi_layer0.self_attn.head_dim
    )
    mi_q_rope, mi_k_rope = mi_layer0.self_attn.rope(
        positions, mi_q_reshaped, mi_k_reshaped
    )
