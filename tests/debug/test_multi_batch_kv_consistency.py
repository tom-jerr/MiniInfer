...
def _encode_prompt(tokenizer, model_name: str, prompt: str) -> List[int]:
    if _should_apply_chat_template(tokenizer, model_name):
        ...
        try:
            ...
            print(
                f"[WARN] apply_chat_template failed ({exc}); fallback to tokenizer.encode"
            )
...
def _filter_finished_from_batch(batch: ScheduledBatch) -> None:
    ...
    if batch.req_pool_indices is not None:
        idx = torch.tensor(
            keep_indices, dtype=torch.int64, device=batch.req_pool_indices.device
        )
        batch.req_pool_indices = batch.req_pool_indices[idx]
    if batch.seq_lens is not None:
        idx = torch.tensor(
            keep_indices, dtype=torch.int64, device=batch.seq_lens.device
        )
...
def _check_kv_cache_layout(
    ...
    for i, req_idx in enumerate(req_indices):
        ...
        if zeros:
            print(
                f"  [WARN] req {i} (pool_idx={req_idx}): zero entries at positions {zeros}"
            )
...
def _compute_next_logits_full_recompute(
    ...
    if debug:
        print(
            f"[DEBUG] input_ids (len={len(input_ids)}): {input_ids[:10]}...{input_ids[-5:]}"
        )
...
def _hf_next_logits(
    ...
    with torch.no_grad():
        logits = hf_model(input_ids=input_tensor, attention_mask=attention_mask).logits[
            0, -1
        ]
...
def run_single_request(
    ...
    kv_locs = (
        kv_mgr.request_pool.read(req.req_pool_idx, slice(0, seq_len))
        .detach()
        .cpu()
        .tolist()
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-Batch KV cache consistency test"
    )
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print detailed debug info"
    )
    parser.add_argument(
        "--compare-hf", action="store_true", help="Also compare against HF model"
    )
    parser.add_argument(
    ...
    for i, prompt_ids in enumerate(all_prompt_ids):
        ...
        if not args.skip_recompute_check:
            check_ok, check_details, recompute_tokens = (
                _check_kv_decode_vs_full_recompute(
                    model_runner=model_runner,
        ...
        if not tokens_match:
            ...
            for j, (st, bt) in enumerate(zip(single_tokens, batch_tokens)):
                if st != bt:
                    ...
                    print(
                        f"    Top-5 single: {[_fmt_token(tokenizer, t.item()) for t in top5_single]}"
                    )
                    print(
                        f"    Top-5 batch:  {[_fmt_token(tokenizer, t.item()) for t in top5_batch]}"
                    )
                    break
            if not mismatch_found and len(single_tokens) != len(batch_tokens):
                print(
                    f"    Length mismatch: single={len(single_tokens)} vs batch={len(batch_tokens)}"
                )
        else:
            ...
            for j in range(len(single_tokens)):
                ...
                close = torch.allclose(
                    single_logits, batch_logits, rtol=1e-2, atol=1e-2
                )
                if not close:
                    print(
                        f"    Step {j} logits NOT close: max_diff={logit_diff.max().item():.4e}"
                    )
                    all_match = False
    ...
    if not args.skip_recompute_check:
        if recompute_all_ok:
            print(
                "✓ Recompute check PASSED: KV decode matches full recompute step-by-step"
            )
        else:
    ...
    if args.compare_hf:
        ...
        for i, prompt_ids in enumerate(all_prompt_ids):
            ...
            print(
                f"  Single tokens: {[_fmt_token(tokenizer, t) for t in single_tokens]}"
            )
            print(
                f"  Batch tokens:  {[_fmt_token(tokenizer, t) for t in batch_tokens]}"
            )
            ...
            if not args.skip_teacher_forcing_check:
                ...
                print(
                    f"  Teacher-forcing align: {'PASS' if tf_ok else 'FAIL'} "
                    f"(steps={len(hf_tokens)})"
                )
                if not tf_ok:
        ...
        if not args.skip_teacher_forcing_check:
            ...
            if teacher_all_ok:
                print(
                    "Teacher-forcing summary: PASS (MiniInfer logits align with HF on forced prefixes)"
                )
            else:
                print(
                    "Teacher-forcing summary: FAIL (MiniInfer vs HF diverge under forced prefixes)"
                )
