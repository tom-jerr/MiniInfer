...
with torch.no_grad():
    ...
    my_embed = my_model.qwen2.embedding(input_ids.reshape(-1)).reshape(
...
if my_residual is None:
    ...
    my_hidden_norm, my_residual = my_model.qwen2.layers[0].input_layernorm(
        my_hidden, my_residual
    )
...
print(
    f"Post norm diff: {torch.abs(hf_hidden_norm2 - my_hidden_norm2_view).max().item()}"
)
print()
