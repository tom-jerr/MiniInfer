...
class StreamOutputFormatter:
    ...
    def print_summary(self):
        ...
        for req_id, progress in sorted(self.progress_map.items()):
            status = self._color("✓", "green") if progress.finished else self._color("○", "yellow")
            print(f"\n{status} Request {req_id}:")
            print(f"   Prompt: {progress.prompt}")
            print(f"   Tokens: {progress.tokens_generated}")
            print(f"   Time: {progress.elapsed_time:.2f}s ({progress.tokens_per_second:.1f} tok/s)")
            print(f"   Output: {progress.text[:100]}{'...' if len(progress.text) > 100 else ''}")
        
        print("\n" + "-" * 60)
        print(f"Total: {self.total_tokens} tokens in {elapsed:.2f}s ({self.total_tokens/elapsed:.1f} tok/s)")
        print("=" * 60)
...
def test_stream_generate_mock():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "expected_tokens": ["Python", " is", " a", " programming", " language", "."],
        },
    ]
...
def test_dynamic_add_request(model_path: str):
    ...
    # 先添加第一个请求
    req_id1 = engine.add_request("第一个问题：1+1=?", sampling_params)
    ...
    while not engine.is_finished():
        ...
        if step_count == 5 and not added_second:
            req_id2 = engine.add_request("第二个问题：2+2=?", sampling_params)
