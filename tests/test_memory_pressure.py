...
class TestRealWorldVsTransformers:
    ...
    def test_real_question_token_precision_vs_transformers(self, engine):
        ...
        assert len(miniinfer_token_ids) == max_new_tokens, (
            f"Expected {max_new_tokens} MiniInfer output tokens, "
            f"got {len(miniinfer_token_ids)}"
        )
        ...
        match_count = 0
        for step_idx, (tid_mi, tid_hf) in enumerate(
            zip(miniinfer_token_ids, hf_token_ids), start=1
        ):
        ...
        print(
            ...
            repr(
                engine.tokenizer.decode(miniinfer_token_ids, skip_special_tokens=False)
            ),
        )
        print(
        ...
        assert match_rate >= min_match_rate, (
            f"Token match rate {match_rate:.2%} is below threshold "
            f"{min_match_rate:.2%}"
        )
