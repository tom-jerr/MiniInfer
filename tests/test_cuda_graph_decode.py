...
def test_cuda_graph_initialization(model_path: str, enforce_eager: bool = False):
    ...
    if enforce_eager:
        assert (
            model_runner.cuda_graph_runner is None
            or not model_runner.cuda_graph_runner.is_available()
        ), "CUDA Graph should NOT be available when enforce_eager=True"
        print("[PASS] CUDA Graph is disabled as expected")
    else:
        assert (
            model_runner.cuda_graph_runner is not None
        ), "CUDA Graph runner should be initialized"
        assert (
...
def test_cuda_graph_correctness(model_path: str):
    ...
    tokens_graph = (
        result_graph.output_token_ids
        if hasattr(result_graph, "output_token_ids")
        else []
    )
    ...
    tokens_eager = (
        result_eager.output_token_ids
        if hasattr(result_eager, "output_token_ids")
        else []
    )
    ...
    if graph_text == eager_text:
        ...
        print(
            f"[WARN] Outputs differ - CUDA Graph: '{graph_text}', Eager: '{eager_text}'"
        )
...
def test_cuda_graph_performance(model_path: str, num_iterations: int = 50):
    ...
    print(
        f"  CUDA Graph:  {graph_time:.3f}s ({graph_time/num_iterations*1000:.1f}ms/iter)"
    )
    print(
        f"  Eager mode:  {eager_time:.3f}s ({eager_time/num_iterations*1000:.1f}ms/iter)"
    )
...
def test_batch_size_padding():
    ...
    for actual, expected in test_cases:
        ...
        if result != expected:
            all_passed = False
        print(
            f"  {status}: _get_padded_batch_size({actual}) = {result} (expected {expected})"
        )
