...
class Qwen2Attention(nn.Module, WeightLoaderMixin):
  def __init__(
    ...
    self.qkv_proj = nn.Linear(config.hidden_size, total_qkv_heads * self.head_dim, bias=True)
  ...
  def forward(
    ...
    query_states = query_states.view(B, L, self.config.num_attention_heads, self.head_dim)
    key_states = key_states.view(B, L, self.config.num_key_value_heads, self.head_dim)
    value_states = value_states.view(B, L, self.config.num_key_value_heads, self.head_dim)
...
class Qwen2MLP(nn.Module, WeightLoaderMixin):
  ...
  def forward(self, x: torch.Tensor) -> torch.Tensor:
    ...
    output = silu_and_mul_forward(gate_up)
